...
Brain::Brain(World *world, EntityRef entity_ref) : m_world(world), m_entity_ref(entity_ref) {
	DASSERT(world);
}
...
ThinkingEntity *Brain::entity() const { return m_world->refEntity<ThinkingEntity>(m_entity_ref); }

Actor *Brain::actor() const { return m_world->refEntity<Actor>(m_entity_ref); }
...
void ActorBrain::onFailed(OrderTypeId) { m_failed_orders++; }
...
void ActorBrain::findActors(int faction_id, const float3 &range, vector<EntityRef> &out) {
	...
	for(int n = 0; n < (int)close_ents.size(); n++) {
		...
		if(nearby && !nearby->isDying())
			if(faction_id == nearby->factionId() ||
			   (faction_id == -1 && isEnemy(nearby->factionId())))
	...
	if(isOneOf(entity->currentOrder(), OrderTypeId::idle, OrderTypeId::track, OrderTypeId::move)) {
		...
		if(actor) {
			...
			if(weapon.needAmmo() && inventory.ammo().count == 0) {
				for(int n = 0; n < inventory.size(); n++)
					if(inventory[n].item.type() == ItemType::ammo &&
					   weapon.canUseAmmo(inventory[n].item)) {
			...
			if(target && !target->isDying()) {
				...
				if(weapon.hasRangedAttack() && can_see) {
					auto mode = (weapon.attackModes() & AttackMode::burst) ? AttackMode::burst :
																			 Maybe<AttackMode>();
			...
			if(target && !target->isDying() && can_see) {
				...
				if(entity->estimateHitChance(Weapon(findProto("_turret_gun", ProtoId::weapon)),
											 target->boundingBox()) > 0.3)
					m_world->sendOrder(new AttackOrder(mode, m_target), m_entity_ref);
			} else { //TODO: change target if cannot do anything about it
				m_target = EntityRef();
			}
...
void ActorBrain::tryRandomMove() {
	...
	if(m_move_delay < 0.0f) {
		...
		float range =
			rand() % 2 && !on_spawn_zone ? 5.0f + frand() * 25.0f : 25.0f + frand() * 150.0f;
...
void ActorBrain::setEnemyFactions(const vector<int> &enemies) { m_enemy_factions = enemies; }

bool ActorBrain::isEnemy(int faction_id) const {
	return m_enemy_factions.empty() ? faction_id != factionId() :
									  isOneOf(faction_id, m_enemy_factions);
}

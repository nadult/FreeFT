...
bool Weapon::canKick() const {
	return isOneOf(classId(), WeaponClass::unarmed, WeaponClass::knife, WeaponClass::pistol,
				   WeaponClass::club);
}
...
bool Weapon::canUseAmmo(const Item &item) const {
	return needAmmo() && item.type() == ItemType::ammo &&
		   Ammo(item).classId() == proto().ammo_class_id;
}
...
float Weapon::estimateDamage(bool include_burst) const {
	...
	if(proto().projectile) {
		...
		return projectile.impact->damage * proto().damage_mod *
			   max(1, include_burst ? proto().burst_ammo : 1);
	}
	if(proto().impact) {

...
PCController::PCController(World &world, const PlayableCharacter &pc) : m_world(world), m_pc(pc) {
...
void PCController::sendOrder(Order *owned_ptr) { sendOrder(POrder(owned_ptr)); }

void PCController::sendOrder(game::POrder &&order) {
	m_world.sendOrder(std::move(order), m_pc.entityRef());
}

bool PCController::hasActor() const { return actor() != nullptr; }

bool PCController::canChangeStance() const { return hasActor(); }
...
void PCController::unequipItem(const Item &item) { sendOrder(new UnequipItemOrder(item.type())); }
...
int PCController::classId() const { return m_pc.classId(); }

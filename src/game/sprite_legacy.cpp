	...
	struct Collection {
		...
		void getLayerIndices(int frame_id, int dir_id, int *layer_indices) const {
			...
			for(int l = 0; l < layer_count; l++)
				layer_indices[l] = type == '1' ? (frame_id * dir_count + dir_id) * 4 + l :
...
template <class InputStream> Ex<void> Sprite::legacyLoad(InputStream &sr, Str) {
	ASSERT(sr.isLoading());
	...
	i16 type;
	sr >> type;
	u8 sizeX, sizeY, sizeZ;
	sr.unpack(sizeZ, sizeY, sizeX);
	m_bbox.x = sizeX;
	m_bbox.y = sizeY;
	m_bbox.z = sizeZ;
	...
	for(int n = 0; n < seq_count; n++) {
		...
		i16 frame_count, dummy1;
		sr.unpack(frame_count, dummy1);
	...
	for(int n = 0; n < collection_count; n++) {
		...
		for(int l = 0; l < Collection::layer_count; l++) {
			...
			palette.colors.insert(palette.colors.end(), collection.palettes[l].begin(),
								  collection.palettes[l].end());
		}

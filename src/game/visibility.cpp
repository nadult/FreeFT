...
void WorldViewer::update(double time_diff) {
	...
	for(int n = 0; n < (int)m_entities.size(); n++) {
		...
		bool is_visible = m_see_all || spectator == entity ||
						  spectator->canSee(entity->ref(), !isMovable(*entity));
		...
		if(is_visible) {
			...
			if(vis_entity.mode == VisEntity::shadowed ||
			   vis_entity.mode == VisEntity::pre_blending_out) {
				...
				vis_entity.mode =
					blend_value > blend_time ? VisEntity::visible : VisEntity::blending_in;
...
const Tile *WorldViewer::refTile(ObjectRef ref) const { return m_world->refTile(ref); }
...
void WorldViewer::addToRender(SceneRenderer &renderer) const {
	...
	for(int n = 0; n < (int)m_entities.size(); n++) {
		...
		const Entity *entity = vis_entity.mode == VisEntity::shadowed ?
								   vis_entity.shadow.get() :
								   m_world->refEntity(vis_entity.ref);
...
Intersection WorldViewer::pixelIntersect(const int2 &screen_pos, const FindFilter &filter) const {
	...
	if(filter.flags() & Flags::tile) {
		const TileMap &tile_map = m_world->tileMap();
		vector<int> inds;
		tile_map.findAll(inds, IRect(screen_pos, screen_pos + int2(1, 1)),
						 filter.flags() | Flags::visible);
	...
	if(filter.flags() & Flags::entity) {
		...
		for(int n = 0; n < (int)m_entities.size(); n++) {
			...
			if(!entity || !m_occluder_config.isVisible(m_entities[n].occluder_id) ||
			   !Flags::test(entity->flags(), filter.flags()) || n == ignore_index)
				continue;

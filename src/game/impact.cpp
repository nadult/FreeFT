...
Impact::Impact(const ImpactProto &proto, EntityRef source, EntityRef target, float damage_mod)
	: EntityImpl(proto), m_played_sound(false), m_applied_damage(false), m_source(source),
	  m_target(target), m_damage_mod(damage_mod) {}
...
XmlNode Impact::save(XmlNode parent) const { return Entity::save(parent); }
...
void Impact::onAnimFinished() { remove(); }
...
void Impact::applyDamage() {
	...
	if(m_proto.type == ImpactType::area || m_proto.type == ImpactType::area_safe) {
		...
		FBox bbox(center - float3(1.0f, 1.0f, 1.0f) * m_proto.range,
				  center + float3(1.0f, 1.0f, 1.0f) * m_proto.range);
		...
		for(int n = 0; n < (int)entities.size(); n++) {
			...
			if(strength > 0.0f) {
				if(distance(center, entity->boundingBox().center()) < big_epsilon) {
					entity->onImpact(m_proto.damage_type, strength * m_proto.damage * m_damage_mod,
									 float3(), m_source);
				} else {
					...
					if(!trace(segment, {Flags::tile | Flags::colliding, entities[n]})) {
						...
						entity->onImpact(m_proto.damage_type,
										 strength * m_proto.damage * m_damage_mod, force, m_source);
					}
				}
			}
		}
...
void Impact::think() {
	...
	if(!m_played_sound) {
		playSound(m_proto.sound_idx, pos(),
				  m_proto.type == ImpactType::area ? SoundType::explosion : SoundType::normal);
		m_played_sound = true;
	}

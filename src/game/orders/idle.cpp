...
IdleOrder::IdleOrder(MemoryStream &sr) : OrderImpl(sr) { sr.unpack(m_fancy_anim_time); }
...
void IdleOrder::cancel() { Order::cancel(); }
...
bool Turret::handleOrder(IdleOrder &order, EntityEvent event, const EntityEventParams &params) {
	...
	if(event == EntityEvent::anim_finished) {
		TurretAction next = m_action == TurretAction::hiding ? TurretAction::hidden :
							m_action == TurretAction::hidden ? TurretAction::hidden :
															   TurretAction::idle;
		animate(next);
	}

...
DropItemOrder::DropItemOrder(Item item, int count) : m_item(item), m_count(count) {}

DropItemOrder::DropItemOrder(MemoryStream &sr) : OrderImpl(sr), m_item(sr) { sr >> m_count; }
...
bool Actor::handleOrder(EquipItemOrder &order, EntityEvent event, const EntityEventParams &params) {
	...
	if(item_type == ItemType::ammo && m_inventory.ammo().item == order.m_item &&
	   m_inventory.ammo().count == m_inventory.weapon().maxAmmo())
		return false;
...
UnequipItemOrder::UnequipItemOrder(MemoryStream &sr) : OrderImpl(sr) { sr >> m_item_type; }
...
bool Actor::handleOrder(UnequipItemOrder &order, EntityEvent event,
...
bool Actor::handleOrder(TransferItemOrder &order, EntityEvent event,
						const EntityEventParams &params) {
	if(order.m_count <= 0)
		return false;

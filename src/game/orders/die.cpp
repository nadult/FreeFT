...
DieOrder::DieOrder(MemoryStream &sr) : OrderImpl(sr) { sr >> m_death_id >> m_is_dead; }
...
bool Actor::handleOrder(DieOrder &order, EntityEvent event, const EntityEventParams &params) {
	...
	if(event == EntityEvent::init_order) {
		bool is_fallen = isOneOf(m_action, Action::fall_forward, Action::fall_back,
								 Action::fallen_back, Action::fallen_forward);
		bool special_death = isOneOf(order.m_death_id, DeathId::explode, DeathId::fire,
									 DeathId::electrify, DeathId::melt);
...
bool Turret::handleOrder(DieOrder &order, EntityEvent event, const EntityEventParams &params) {
	...
	if(event == EntityEvent::sound) {
		auto sound_id = order.m_death_id == DeathId::explode ? TurretSoundId::death_explode :
															   TurretSoundId::death;

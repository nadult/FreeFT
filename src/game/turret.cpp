...
static const EnumMap<TurretAction, const char *> s_seq_names = {
	{"StandArmed", "StandAttackUnarmedTwo", "StandAttackUnarmedThree", "StandUnarming",
	 "StandArming", "StandUnarmed", "Death", "DeathElectrify", "DeathExplode"}};
...
static const EnumMap<TurretSoundId, TurretSound> s_sounds = {
	{{"death", 0}, {"death", 1}, {"attack", 1}, {"attack", 2}, {"arming", 0}, {"unarming", 0}}};
...
Turret::Turret(MemoryStream &sr) : EntityImpl(sr) {
	m_hit_points = decodeInt(sr);
	sr.unpack(m_target_angle, m_action);
}

Turret::Turret(CXmlNode node) : EntityImpl(node), m_target_angle(dirAngle()) {
	m_hit_points = m_proto.hit_points;
	animate(TurretAction::idle);
}

Turret::Turret(const Proto &proto) : EntityImpl(proto), m_target_angle(dirAngle()) {
	m_hit_points = m_proto.hit_points;
	animate(TurretAction::idle);
}

XmlNode Turret::save(XmlNode parent) const { return EntityImpl::save(parent); }
...
FlagsType Turret::flags() const { return Flags::turret | Flags::dynamic_entity | Flags::colliding; }
...
void Turret::think() { ThinkingEntity::think(); }
...
bool Turret::isDying() const { return m_order && m_order->typeId() == OrderTypeId::die; }
...
bool Turret::animateDeath(DeathId death) {
	auto anim_id = death == DeathId::explode   ? TurretAction::death_explode :
				   death == DeathId::electrify ? TurretAction::death_electrify :
												 TurretAction::death;

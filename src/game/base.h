...
DEFINE_ENUM(WeaponClass, unarmed, club, heavy, knife, minigun, pistol, rifle, rocket, smg, spear);
...
namespace Flags {
	...
	inline constexpr bool test(unsigned object_flags, Type test) {
		return (object_flags & test & 0xffffff) &&
			   ((object_flags & test & 0xff000000) == (test & 0xff000000));
	}

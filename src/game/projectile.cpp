...
void ProjectileProto::link() { impact.link(); }

Projectile::Projectile(const ProjectileProto &proto, float initial_angle, const float3 &dir,
					   EntityRef spawner, float damage_mod)
	: EntityImpl(proto), m_dir(dir), m_spawner(spawner), m_distance(0.0f), m_damage_mod(damage_mod),
	  m_impact_created(false) {
...
Projectile::Projectile(MemoryStream &sr) : EntityImpl(sr) {
	sr.unpack(m_dir, m_speed, m_frame_count, m_target_angle, m_damage_mod, m_distance,
			  m_impact_created);
	m_spawner.load(sr);
}
...
void Projectile::save(MemoryStream &sr) const {
	EntityImpl::save(sr);
	sr.pack(m_dir, m_speed, m_frame_count, m_target_angle, m_damage_mod, m_distance,
			m_impact_created);
	m_spawner.save(sr);
}

XmlNode Projectile::save(XmlNode parent) const { return Entity::save(parent); }
...
void Projectile::think() {
	...
	Intersection isect =
		trace(Segment3F(ray.origin(), ray.at(ray_pos)), {Flags::all | Flags::colliding, m_spawner});
...
void Projectile::onAnimFinished() { remove(); }

...
GameMode::GameMode(World &world, int current_id) : m_world(world), m_current_id(current_id) {
	if(!m_world.isClient())
		initAISpawnZones();
}
	...
	for(auto &spawn : m_spawn_zones) {
		...
		if(spawn.next_spawn_time <= 0.0) {
			...
			if(idx != -1) {
				...
				EntityRef ai = spawnActor(spawn.ref, getProto("rad_scorpion", ProtoId::actor),
										  ActorInventory());
...
EntityRef GameMode::spawnActor(EntityRef spawn_zone_ref, const Proto &proto,
	...
	int it = 0, it_max = 100;
	for(; it < it_max; it++) {
		spawn_pos = spawn_box.min() +
					float3(frand() * spawn_box.width(), 1.0f, frand() * spawn_box.depth());
...
GameModeServer::GameModeServer(World &world) : GameMode(world, -1) { ASSERT(world.isServer()); }

void GameModeServer::tick(double time_diff) { GameMode::tick(time_diff); }
...
void GameModeClient::tick(double time_diff) { GameMode::tick(time_diff); }
...
void GameModeClient::onClientDisconnected(int client_id) { m_clients.erase(client_id); }

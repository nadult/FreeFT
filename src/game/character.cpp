...
int CharacterClass::count() { return (int)s_classes.size(); }
...
PlayableCharacter::PlayableCharacter(MemoryStream &sr)
	: m_character(sr), m_class_id(CharacterClass::defaultId()) {
...
void PlayableCharacter::load(MemoryStream &sr) { *this = PlayableCharacter(sr); }

bool PlayableCharacter::operator==(const PlayableCharacter &rhs) const {
	return m_character == rhs.m_character && m_class_id == rhs.m_class_id &&
		   m_entity_ref == rhs.m_entity_ref;
}

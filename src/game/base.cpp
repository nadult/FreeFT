...
static const EnumMap<HealthStatus, const char *> s_health_desc = {
	{"unhurt", "barely wounded", "wounded", "seriously wounded", "near death", "dead"}};

const char *describe(HealthStatus status) { return s_health_desc[status]; }

HealthStatus healthStatusFromHP(float health) {
	return health > 0.99f ? HealthStatus::unhurt :
		   health > 0.70f ? HealthStatus::barely_wounded :
		   health > 0.50f ? HealthStatus::wounded :
		   health > 0.20f ? HealthStatus::seriously_wounded :
		   health > 0.0f  ? HealthStatus::near_death :
							HealthStatus::dead;
}

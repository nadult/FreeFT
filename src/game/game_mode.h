...
DEFINE_ENUM(UserMessageType, main, log);
...
struct PCIndex {
	PCIndex(int client_id, int pc_id) : client_id(client_id), pc_id(pc_id) {}
...
class GameMode {
  public:
	...
	const GameClient *client(int client_id) const {
		return const_cast<GameMode *>(this)->client(client_id);
	}
	const PlayableCharacter *pc(const PCIndex &index) const {
		return const_cast<GameMode *>(this)->pc(index);
	}

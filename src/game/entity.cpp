...
Entity::Entity(const Sprite &sprite) : m_sprite(sprite), m_pos(0.0f, 0.0f, 0.0f) {
	resetAnimState();
}
...
static constexpr int flag_compressed = 1, flag_is_looped = 2, flag_is_finished = 4,
					 flag_has_overlay = 8;
...
void Entity::save(MemoryStream &sr) const {
	...
	bool can_compress = m_seq_idx >= 0 && m_seq_idx <= 255 && m_frame_idx >= 0 &&
						m_frame_idx <= 255 && m_dir_idx >= 0 && m_dir_idx <= 255;

	u8 flags = (can_compress ? flag_compressed : 0) | (m_is_seq_looped ? flag_is_looped : 0) |
...
const FBox Entity::boundingBox() const { return FBox(m_pos, m_pos + float3(m_sprite.bboxSize())); }
...
bool Entity::testPixel(const int2 &screen_pos) const {
	return m_sprite.testPixel(screen_pos - (int2)worldToScreen(pos()), m_seq_idx, m_frame_idx,
							  m_dir_idx);
}
	...
	if(seq_idx != m_seq_idx || !m_is_seq_looped) {
		...
		while(m_frame_idx < frame_count && m_sprite.frame(seq_idx, m_frame_idx).id < 0) {
			...
			if(frame_id == Sprite::ev_overlay && m_oseq_idx != -1) {
				...
				while(m_oframe_idx < ov_frame_count &&
					  m_sprite.frame(m_oseq_idx, m_oframe_idx).id < 0)
					m_oframe_idx++;

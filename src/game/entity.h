...
class FindFilter {
  public:
	...
	FindFilter(FlagsType flags, ObjectRef ignore) : m_flags(flags), m_ignore_object_ref(ignore) {
		DASSERT(ignore.isEntity());
	}
	FindFilter(EntityRef ignore) : m_flags(Flags::all), m_ignore_entity_ref(ignore) {}
	FindFilter(ObjectRef ignore) : m_flags(Flags::all), m_ignore_object_ref(ignore) {
		DASSERT(ignore.isEntity());
	}
...
class EntityImpl : public Base {
  private:
	...
	EntityImpl(const Initializer &init) : Base(*init.sprite), m_proto(*init.proto) {}
  ...
  public:
	...
	virtual Entity *clone() const { return new Type(*static_cast<const Type *>(this)); }
	virtual EntityId typeId() const { return entity_id; }
...
template <class TEntity, class TRef> TEntity *EntityWorldProxy::refEntity(TRef ref) {

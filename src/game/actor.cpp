...
Actor::Actor(CXmlNode node)
	: EntityImpl(node), m_actor(*m_proto.actor), m_stance(Stance::stand),
	  m_inventory(node.child("inventory")), m_target_angle(dirAngle()), m_client_id(-1) {
	m_inventory.setDummyWeapon(Weapon(*m_actor.punch_weapon));
...
static const Proto &getActorArmour(const Proto &actor, ActorInventory &inventory) {
	...
	if(inventory.isEquipped(ItemType::weapon) &&
	   !proto.canEquipWeapon(inventory.weapon().classId()))
		inventory.unequip(ItemType::weapon);

	return proto;
}

Actor::Actor(const Proto &proto, ActorInventory &inv)
	: EntityImpl(getActorArmour(proto, inv)), m_actor(*m_proto.actor), m_stance(Stance::stand),
	  m_inventory(inv), m_target_angle(dirAngle()), m_client_id(-1) {
	m_inventory.setDummyWeapon(Weapon(*m_actor.punch_weapon));
	m_sound_variation = rand() % m_actor.sounds.size();
	m_faction_id = 0;
	m_hit_points = m_actor.hit_points;
...
float Actor::fallChance(DamageType type, float damage, const float3 &force_vec) const {
	float force = length(force_vec) *
					  (m_action == Action::walk ? 1.25f :
					   m_action == Action::run	? 1.5f :
												  1.0f) *
					  0.2f -
				  0.5f;
...
void Actor::onImpact(DamageType damage_type, float damage, const float3 &force, EntityRef source) {
	...
	GetHitOrder *current = m_order && m_order->typeId() == OrderTypeId::get_hit ?
							   static_cast<GetHitOrder *>(m_order.get()) :
							   nullptr;

	bool is_fallen = current && (current->mode == GetHitOrder::Mode::fall ||
								 current->mode == GetHitOrder::Mode::fallen);
	...
	if(m_hit_points <= 0.0f) {
		...
		if(will_fall) {
			...
			if(current_type_id != OrderTypeId::change_stance &&
			   (current_type_id == OrderTypeId::idle || random() > 0.5f))
...
WeaponClass Actor::equippedWeaponClass() const { return m_inventory.weapon().classId(); }
...
bool Actor::canSee(EntityRef target_ref, bool simple_test) {
	...
	return world()->isVisible(eye_pos, target_ref, ref(),
							  target->typeId() == EntityId::actor ? 4 : 3);
}
...
bool Actor::canChangeStance() const { return m_proto.canChangeStance(); }
...
bool Actor::isDying() const { return m_order && m_order->typeId() == OrderTypeId::die; }
...
const float3 Actor::estimateMove(float time_advance) const { return m_move_vec * time_advance; }
...
void Actor::fireProjectile(const FBox &target_box, const Weapon &weapon, float randomness) {
	...
	if(const ProjectileProto *proj_proto = weapon.projectileProto())
		addNewEntity<Projectile>(best_ray.origin(), *proj_proto, actualDirAngle(), best_ray.dir(),
								 ref(), weapon.proto().damage_mod);
}
...
void Actor::addToRender(SceneRenderer &out, Color color) const {
	...
	const AttackOrder *attack = m_order && m_order->typeId() == OrderTypeId::attack ?
									static_cast<AttackOrder *>(m_order.get()) :
									nullptr;
	if(attack) {
		const Actor *target = const_cast<Actor *>(this)->refEntity<Actor>(attack->m_target);
		if(target) {
			FBox bounding_box =
				target->boundingBox() + attack->m_target_pos - target->boundingBox().center();

...
namespace parser
{
	...
	namespace
	{
		...
		typedef position_iterator<char const*> Iterator;
		Iterator *tFirstIter;
		...
		struct Types: symbols<TYPE>
			{ Types() { for(u32 n=0;types[n].str;n++) add(types[n].str,types[n].id); } } types_p;

		struct Javalette: public grammar<Javalette>
		{
			#define EXPECT( op , errMsg )		assertion<const char*>(errMsg " expected") ( op )
		...
		/*		funcArg =
			...
			rule<ScannerT> identifier,type,keyword;
			rule<ScannerT> expression,exprLogicalAnd,exprCompare,exprRelation,exprAddition,exprMul,exprUnary,exprSimple;
			rule<ScannerT> instruction,instrList,instrDeclare,declarator,instrSet;
			rule<ScannerT> first,program,funcDef,funcArg;

			guard<const char*> programGuard,funcGuard,instrGuard;

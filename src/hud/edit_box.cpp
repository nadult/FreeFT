...
void HudEditBox::onInputFocus(bool is_focused) { setEnabled(is_focused); }
...
bool HudEditBox::onInput(const InputEvent &event) {
	...
	if(!isEnabled() && event.mouseButtonDown(InputButton::left) && !isGreyed() &&
	   isMouseOver(event)) {
		setInputFocus(true);
		handled = true;
		m_old_text = m_text;
			...
			if(event.type() == InputEventType::key_char) {
				...
				if(isValidChar(key) && (int)m_text.size() < m_max_size)
					m_text.insert(m_cursor_pos++, 1,
								  m_mode == mode_locase_nick ? tolower(key) : key);
...
void HudEditBox::onDraw(Renderer2D &out) const {
	...
	FRect target_rect = m_font->draw(out, rect() + float2(layer_spacing, 0.0f),
									 {textColor(), textShadowColor(), HAlign::left, VAlign::center},

...
const FRect align(const FRect &rect, const FRect &relative_to, Alignment mode, float spacing) {
	...
	if(mode >= align_top_left) {
		if(mode == align_top_left)
			return align(align(rect, relative_to, align_left, spacing), relative_to, align_top,
						 spacing);
		else if(mode == align_top_right)
			return align(align(rect, relative_to, align_right, spacing), relative_to, align_top,
						 spacing);
		else if(mode == align_down_left)
			return align(align(rect, relative_to, align_left, spacing), relative_to, align_down,
						 spacing);
		else /*if(mode == align_down_right)*/
			return align(align(rect, relative_to, align_right, spacing), relative_to, align_down,
						 spacing);
	}
...
static EnumMap<HudSound, const char *> s_sounds = {
	{"", "butn_text", "butn_itemswitch", "butn_optionknob"}};
...
HudStyle getStyle(HudStyleId style_id) {
	if(style_id == HudStyleId::green_white)
		return HudStyle{Color(150, 200, 150), Color(30, 255, 60), Color(30, 255, 60),
						ColorId::white,		  "transformers_20",  "transformers_30"};
	else if(style_id == HudStyleId::red_green)
		return HudStyle{ColorId::green, Color(255, 60, 30), Color(255, 60, 30),
						ColorId::white, "transformers_20",	"transformers_30"};
	else //if(style_id == HudStyleId::console)
		return HudStyle{ColorId::green, Color(255, 60, 30), Color(255, 60, 30),
						ColorId::white, "liberation_16",	"liberation_24"};
}
}

...
HudStats::HudStats(const FRect &target_rect) : HudLayer(target_rect) {
	setTitle("Statistics:");
...
bool HudStats::canShow() const { return m_pc_controller && m_pc_controller->world().isClient(); }
...
void HudStats::updateData() {
	...
	if(m_pc_controller) {
		const GameModeClient *game_mode =
			dynamic_cast<const GameModeClient *>(m_pc_controller->world().gameMode());
		if(game_mode) {

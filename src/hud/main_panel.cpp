...
namespace {
	...
	static Button s_buttons[] = {{layer_inventory, "INV"},
	...
	static StanceButton s_stance_buttons[] = {{Stance::prone, HudIcon::stance_prone},
...
HudMainPanel::HudMainPanel(const FRect &rect) : HudLayer(rect) {
...

	{
		...
		for(int n = 0; n < arraySize(s_stance_buttons); n++) {
			PHudButton stance(
				new HudRadioButton(stance_rect, (int)s_stance_buttons[n].stance_id, 1));

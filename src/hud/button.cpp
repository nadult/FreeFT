...
namespace {
	...
	static const EnumMap<HudIcon, IconInfo> s_icons = {{{FRect(0.00f, 0.00f, 0.25f, 0.25f)},
...
HudButton::HudButton(const FRect &rect, int id)
	: HudWidget(rect), m_button_style(HudButtonStyle::normal), m_label_style(HudLabelStyle::center),
	  m_id(id) {
	m_icons_tex = res::getTexture("icons.png", true);
}
...
void HudButton::onDraw(Canvas2D &out) const {
	...
	if(!m_label.empty() && m_label_style != HudLabelStyle::disabled) {
		auto halign = m_label_style == HudLabelStyle::left	 ? HAlign::left :
					  m_label_style == HudLabelStyle::center ? HAlign::center :
															   HAlign::right;
		FRect font_rect = rect.inset(float2(layer_spacing, 0.0f));
		m_font->draw(out, font_rect, {textColor(), textShadowColor(), halign, VAlign::center},
					 m_label);
	}
...
bool HudButton::onInput(const InputEvent &event) {
	...
	if((event.mouseButtonDown(InputButton::left) && isMouseOver(event)) ||
	   (m_accelerator && event.keyDown(m_accelerator))) {
		onClick();
		return true;
	}
...
Color HudButton::textColor(bool force_enabled) const {
	auto fcolor = lerp((FColor)Color(m_style.enabled_color, 160), FColor(m_style.enabled_color),
					   force_enabled ? 1.0f : m_enabled_time);
	fcolor.a *= alpha();
	return (Color)fcolor;
}

Color HudButton::textShadowColor() const { return (Color)mulAlpha(ColorId::black, alpha()); }
...
Color HudButton::borderColor() const {
	u8 border_alpha =
		clamp((int)(255 * alpha() * (0.3f + 0.7f * m_enabled_time * m_highlighted_time)), 0, 255);
...
HudToggleButton::HudToggleButton(const FRect &target_rect, int id) : HudButton(target_rect, id) {}
...
bool HudRadioButton::onInput(const InputEvent &event) {
	...
	bool clicked = (event.mouseButtonDown(InputButton::left) && isMouseOver(event)) ||
				   (m_accelerator && event.keyDown(m_accelerator));

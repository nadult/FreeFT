...
bool HudItemEntry::operator<(const HudItemEntry &rhs) const {
	return item.type() == rhs.item.type() ? item.name() < rhs.item.name() :
											item.type() < rhs.item.type();
}
...
HudItemDesc::HudItemDesc(const FRect &rect) : HudLayer(rect) { m_anim_speed = 10.0f; }

const FRect HudItemDesc::rect() const { return targetRect(); }

float HudItemDesc::alpha() const { return m_visible_time * HudLayer::alpha(); }
...
void HudItemDesc::onDraw(Canvas2D &out) const {
	...
	if(!m_item.isDummy()) {
		...
		m_font->draw(out, float2(rect.x() + 5.0f, ypos), {titleColor(), titleShadowColor()},
					 params_desc);
	}
}
	...
	if(!m_entry.item.isDummy()) {
		...
		if(m_entry.count > 1)
			m_font->draw(out, rect, {textColor(), textShadowColor(), HAlign::right},
						 format("%", m_entry.count));
		if(isDropping())
			m_font->draw(out, rect,
						 {textColor(true), textShadowColor(), HAlign::left, VAlign::bottom},
						 format("-%", dropCount()));
	}
}
...
void HudItemButton::onUpdate(double time_diff) {
	...
	if(isDropping()) {
		m_drop_count +=
			(m_drop_diff < 0.0f ? -1.0f : 1.0f) * (m_drop_diff * m_drop_diff) * time_diff;
...
HudInventory::HudInventory(const FRect &target_rect)
	...
	for(int y = 0; y < s_grid_size.y; y++)
		for(int x = 0; x < s_grid_size.x; x++) {
			float2 pos(s_item_size.x * x + item_spacing * (x + 1),
					   s_item_size.y * y + item_spacing * (y + 1) + topOffset());
	...
	m_item_desc =
		make_shared<HudItemDesc>(FRect(s_item_desc_size) + float2(layer_spacing, layer_spacing));
	m_item_desc->setVisible(false, false);
}

HudInventory::~HudInventory() {}

bool HudInventory::canShow() const { return m_pc_controller && m_pc_controller->actor(); }
...
void HudInventory::updateData() {
	...
	for(int n = 0; n < temp.size(); n++) {
		bool is_equipped = inventory.armour() == temp[n].item ||
						   inventory.weapon() == temp[n].item ||

...
HudWidget::HudWidget(const FRect &rect)
	: m_parent(nullptr), m_input_focus(nullptr), m_rect(rect), m_visible_time(1.0),
	  m_anim_speed(10.0f), m_is_visible(true), m_needs_layout(true) {
...
bool HudWidget::handleInput(const InputEvent &event) {
	if(!m_is_visible)
		if(!isOneOf(event.type(), InputEventType::key_up, InputEventType::mouse_button_up,
					InputEventType::mouse_over))
			return false;
...
bool HudWidget::isVisible() const { return m_is_visible || m_visible_time > 0.01f; }

bool HudWidget::isShowing() const { return m_is_visible && m_visible_time < 1.0f; }

bool HudWidget::isHiding() const { return !m_is_visible && m_visible_time > 0.01f; }

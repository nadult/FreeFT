...
EntityPad::EntityPad(const IRect &max_rect, EntityId type_id)
	: Window(IRect(max_rect.min(), int2(max_rect.ex(), max_rect.y() + 1))), m_max_rect(max_rect),
	  m_type_id(type_id) {}
...
PEntity ContainerPad::makeEntity() const {
	...
	return Dynamic<Container>(proto);
}
...
PEntity ItemPad::makeEntity() const {
	...
	return Dynamic<ItemEntity>(Item(index), m_count_val);
}
...
void ItemPad::updateItemIds() {
	...
	for(int n = 0; n < countProtos(proto_id); n++) {
		const game::ItemProto &proto = static_cast<const game::ItemProto &>(getProto(n, proto_id));
		if(!proto.is_dummy)
...
PEntity TriggerPad::makeEntity() const {
	Dynamic<Trigger> out;
	out.emplace<Trigger>((TriggerClassId)m_class_id->selectedId(), FBox(0, 0, 0, 1, 1, 1));
...
EntitiesPad::EntitiesPad(const IRect &rect, PEntitiesEditor editor)
	...
	m_editor_mode_box =
		make_shared<ComboBox>(IRect(0, 0, width, WindowStyle::line_height), 200, "Editing mode: ");
	for(auto mode : all<Mode>)
		m_editor_mode_box->addEntry(describe(mode));
	m_editor_mode_box->selectEntry((int)m_editor->mode());

	m_entity_type = make_shared<ComboBox>(
		IRect(0, WindowStyle::line_height, width, WindowStyle::line_height * 2), 200,
		"Entity type: ");
...
void EntitiesPad::updateEntity() { m_editor->setProto(makeEntity()); }

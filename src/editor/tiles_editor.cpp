...
void TilesEditor::onInput(const InputState &state) {
	...
	if(state.isKeyDown('s'))
		setMode(isSelecting() && m_mode != mode_selecting_difference ? (Mode)(m_mode + 1) :
	...
	m_current_occluder =
		m_mouseover_tile_id == -1 ? -1 : m_tile_map[m_mouseover_tile_id].occluder_id;
...
void TilesEditor::fill(const IBox &fill_box, bool is_randomized, int group_id) {
	...
	if(isReplacing()) {
		...
		col_box = {col_box.min(),
				   col_box.min() + int3(col_box.width() / bbox.x, col_box.height() / bbox.y,
										col_box.depth() / bbox.z) *
									   bbox};
		removeAll(col_box);
	}
		...
		for(int z = fill_box.z(); z < fill_box.ez(); z += bbox.z) {
			...
			if(is_randomized) {
				const vector<int> &source =
					rand() % 1000 < 1000 * m_dirty_percent ? dirty_entries : entries;
...
void TilesEditor::fillHoles(int main_group_id, const IBox &fill_box) {
	...
	for(int x = fill_box.x(); x < fill_box.ex(); x += bbox.x)
		for(int z = fill_box.z(); z < fill_box.ez(); z += bbox.z) {
			...
			int neighbours[8] = {
				findAt(pos + int3(0, 0, bbox.z)),		 findAt(pos + int3(bbox.x, 0, 0)),
				findAt(pos - int3(0, 0, bbox.z)),		 findAt(pos - int3(bbox.x, 0, 0)),

				findAt(pos + int3(bbox.x, 0, bbox.z)),	 findAt(pos + int3(bbox.x, 0, -bbox.z)),
				findAt(pos + int3(-bbox.x, 0, -bbox.z)), findAt(pos + int3(-bbox.x, 0, bbox.z)),
			};
			...
			for(int n = 0; n < 8; n++) {
				...
				if(n < 4) {
					for(int s = 0; s < 3; s++) {
						int src_surf =
							m_tile_group->groupSurface(ngroups[n], (soffset[n] - s + 8) % 8);
			...
			for(int n = 0; n < arraySize(sides); n++) {
				...
				if(sides[n] == -1 && (next == -1 || next == main_surf) &&
				   (prev == -1 || prev == main_surf))
					sides[n] = main_surf;
			}
...
bool TilesEditor::onMouseDrag(const InputState &state, int2 start, int2 current, int key,
							  int is_final) {
	if(key == 0 && !state.isKeyPressed(InputKey::lctrl) && !isChangingOccluders()) {
		...
		if(is_final && is_final != -1) {
			if(isSelecting()) {
				vector<int> new_ids;
				m_tile_map.findAll(new_ids, FBox((float3)m_selection.min(),
												 float3(m_selection.max() + int3(0, 1, 0))));
				makeSorted(new_ids);
		...
		return true;
			  ...
			  (m_mode >= mode_selecting_normal && m_mode <= mode_selecting_difference)) {
		if(!m_is_moving) {
		...
		if(is_final > 0) {
			...
			for(int n = 0; n < (int)m_selected_ids.size(); n++) {
				...
				if(bbox.x() < 0 || bbox.y() < 0 || bbox.z() < 0 || bbox.ex() > dims.x ||
				   bbox.ey() >= Grid::max_height || bbox.ez() > dims.y)
					cancel = true;
			}

			std::sort(temp.begin(), temp.end());
			temp.resize(std::unique(temp.begin(), temp.end()) - temp.begin());
			temp.resize(std::set_difference(temp.begin(), temp.end(), m_selected_ids.begin(),
											m_selected_ids.end(), temp.begin()) -
						temp.begin());
...
void TilesEditor::drawContents(Canvas2D &out) const {
...

	{
		...
		visible_ids.resize(std::set_difference(visible_ids.begin(), visible_ids.end(),
											   m_selected_ids.begin(), m_selected_ids.end(),
											   visible_ids.begin()) -
						   visible_ids.begin());
		...
		if(isChangingOccluders()) {
			...
			Color colors[] = {Color(255, 200, 200), Color(255, 255, 200), Color(255, 200, 255),
							  Color(200, 255, 255), Color(200, 255, 200), Color(200, 200, 255)};
			...
			if(selected_occluder != -1) {
				...
				for(int n = 0; n < occmap.size(); n++)
					is_occluder_selected[n] =
						n == selected_occluder ? 1 : occmap.isUnder(selected_occluder, n);
			...
			for(int n = 0; n < (int)visible_ids.size(); n++) {
				...
				Color col = box.ey() < m_selection.y()	? ColorId::gray :
							box.ey() == m_selection.y() ? Color(200, 200, 200, 255) :
														  ColorId::white;
	...
	font.draw(out, float2(0, clippedRect().height() - 25), {ColorId::white, ColorId::black},
			  stdFormat("Cursor: (%d, %d, %d)  Grid: %d Mode: %s\n", m_selection.x(),
						m_selection.y(), m_selection.z(), m_view.gridHeight(),
						s_mode_strings[m_mode]));
}

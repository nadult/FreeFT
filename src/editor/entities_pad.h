...
class EntityPad : public Window {
  public:
	...
	template <class TControl, class... Args> shared_ptr<TControl> addControl(const Args &...args) {
		IRect pad_rect = rect();
		IRect crect(0, pad_rect.height(), pad_rect.width(),
					pad_rect.height() + WindowStyle::line_height);

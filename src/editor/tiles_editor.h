...
class TilesEditor : public ui::Window {
  public:
	...
	bool isReplacing() const { return m_mode == mode_replacing || m_mode == mode_replacing_random; }
	bool isPlacing() const { return m_mode >= mode_placing && m_mode <= mode_replacing; }
	bool isPlacingRandom() const {
		return m_mode >= mode_placing_random && m_mode <= mode_replacing_random;
	}
	bool isFilling() const { return m_mode == mode_filling; }
	bool isChangingOccluders() const { return m_mode == mode_occluders; }

...
namespace ui {
		...
		for(int gy = m_selection.y(); gy <= m_selection.ey(); gy += block_size)
			for(int gx = m_selection.x(); gx <= m_selection.ex(); gx += block_size) {
				...
				for(int y = gy, endy = min(m_selection.ey(), gy + block_size); y <= endy; y += 2)
					for(int x = m_selection.x(), endx = min(gx + block_size, m_selection.ex()); x <= endx; x += 2) {
						int2 point(x, y);
				...
				if(m_proto->typeId() == EntityId::trigger) {
					...
					if(m_trigger_mode == 1) {
						...
						if(offset < 0)
							...
							new_box = {new_box.min(), {new_box.ex(), new_box.ey() + offset, new_box.ez()}};
					}
	...
	FBox EntitiesEditor::computeOvergroundBox(const FBox &bbox) const {
		...
		if(over_ground)
			return FBox(asXZY(bbox.min().xz(), bbox.y() - over_ground), asXZY(bbox.max().xz(), bbox.y()));
		return FBox();
	}

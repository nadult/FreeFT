...
void Controller::onInput(const InputEvent &event) {
	...
	if((event.mouseButtonPressed(InputButton::left) && event.pressed(InputModifier::lctrl)) ||
	...
	if(event.isMouseOverEvent() && mouse_over_hud) {
		...
		FlagsType flags = Flags::walkable_tile |
		...
		if(entity) {
			...
			bool run =
				actor &&
				!event.pressed(
					InputModifier::lshift); // && distance(float3(wpos), actor->pos()) > 10.0f;
...
void Controller::drawDebugInfo(Canvas2D &out) const {
	...
	out.addFilledRect(FRect((float2)pos, (float2)out.viewport().size()),
					  FColor(0.0f, 0.0f, 0.0f, 0.3f));

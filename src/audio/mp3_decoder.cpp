...
MP3Decoder::MP3Decoder(const string &file_name) {
	m_stream.emplace(std::move(fileLoader(file_name).get())); // TODO: pass it properly
...
bool MP3Decoder::decode(Sound &out, int max_size) {
	...
	do {
		...
		if(m_need_data) {
			...
			ret =
				mpg123_decode(handle, 0, 0, (u8 *)temp.data() + out_pos, max_size - out_pos, &size);
		}
	...
	out = {std::move(out_data), {m_sample_rate, 16, m_num_channels == 2}};

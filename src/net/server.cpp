...
bool ServerConfig::isValid() const { return !m_server_name.empty() && !m_map_name.empty(); }
...
Server::Server(const ServerConfig &config)
	: LocalHost(Address(config.m_port)), m_config(config), m_game_mode(nullptr) {
	...
	while(const Chunk *chunk_ptr = host.getIChunk()) {
		...
		if(chunk.type() == ChunkType::join && client.mode == ClientMode::connecting) {
			...
			if(!m_config.m_password.empty() && password != m_config.m_password) {
				...
				for(int n = 0; n < (int)m_clients.size(); n++)
					if(n != client_id && m_clients[n].isValid() &&
					   m_clients[n].nick_name == client.nick_name) {
	...
	if(client.mode == ClientMode::connected) {
		...
		while(idx < m_world->entityCount()) {
			...
			if(map[idx]) {
				...
				if(host.enqueUChunk(temp.data(),
									entity ? ChunkType::entity_full : ChunkType::entity_delete, idx,
									1))
					map[idx] = false;
				else
					break;
...
void Server::replicateEntity(int entity_id) { m_replication_list.emplace_back(entity_id); }

...
namespace {
	...
	void logEnd(int packet_size) {
		...
		printf("%s(size:%d id:%d)[ ", s_log_info.is_sending ? "OUT" : " IN", packet_size,
			   s_log_info.packet_id);
...
#define INSERT(list, idx)                                                                          \
	listInsert([this](int i) -> ListNode & { return m_chunks[i].m_node; }, list, idx)
#define REMOVE(list, idx)                                                                          \
	listRemove([this](int i) -> ListNode & { return m_chunks[i].m_node; }, list, idx)

#define U_INSERT(list, idx)                                                                        \
	listInsert([this](int i) -> ListNode & { return m_uchunks[i].node; }, list, idx)
#define U_REMOVE(list, idx)                                                                        \
	listRemove([this](int i) -> ListNode & { return m_uchunks[i].node; }, list, idx)

RemoteHost::RemoteHost(const Address &address, int max_bytes_per_frame, int current_id,
					   int remote_id)
...
double RemoteHost::timeout() const { return getTime() - m_last_time_received; }
...
void RemoteHost::newPacket(bool is_first) {
	...
	PacketInfo(m_out_packet_id, m_current_id, m_remote_id, mask(is_first, PacketFlag::first))
		.save(m_out_packet);
...
bool RemoteHost::sendUChunk(int chunk_idx, CSpan<char> data, ChunkType type) {
	...
	DASSERT(type != ChunkType::ack && type != ChunkType::invalid &&
			type != ChunkType::multiple_chunks);
...
int RemoteHost::allocChunk() {
	return freeListAlloc([&](int idx) -> ListNode & { return m_chunks[idx].m_node; }, m_chunks,
						 m_free_chunks);
}
int RemoteHost::allocUChunk() {
	return freeListAlloc([&](int idx) -> ListNode & { return m_uchunks[idx].node; }, m_uchunks,
						 m_free_uchunks);
}
...
void RemoteHost::beginSending(Socket *socket) {
	...
	if(num_acks) {
		...
		for(int i = 1; i < num_acks;) {
			...
			int count = 1;
			while(i + count < num_acks &&
				  int(m_out_acks[i + count]) == int(m_out_acks[i + count - 1]) + 1)
				count++;
...
void RemoteHost::handlePacket(InPacket &packet) {
	...
	while(!packet.atEnd()) {
		...
		if(data_size < 0 || data_size > PacketInfo::max_size || channel_id < 0 ||
		   channel_id >= (int)m_channels.size())
			goto ERROR;
...
void RemoteHost::finishReceiving() {
	...
	std::sort(m_current_ichunk_indices.begin(), m_current_ichunk_indices.end(),
			  OrderIChunks(m_chunks.data()));
	std::merge(m_ichunk_indices.begin(), m_ichunk_indices.end(), m_current_ichunk_indices.begin(),
			   m_current_ichunk_indices.end(), temp.begin());
	...
	m_ichunk_indices.resize(std::remove(m_ichunk_indices.begin(), m_ichunk_indices.end(), -1) -
							m_ichunk_indices.begin());
...
bool RemoteHost::canFit(int data_size) const { return estimateSize(data_size) <= m_bytes_left; }

int RemoteHost::estimateSize(int data_size) const { return data_size + 8; }

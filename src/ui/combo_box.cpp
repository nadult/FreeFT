...
int ComboBox::findEntry(const char *text) const { return m_dummy->findEntry(text); }

int ComboBox::selectedId() const { return (m_popup ? m_popup : m_dummy)->selectedId(); }
...
const ListBox::Entry &ComboBox::operator[](int idx) const { return m_dummy->operator[](idx); }
...
int ComboBox::size() const { return m_dummy->size(); }

void ComboBox::updateButton() {
	int selected_id = m_dummy->selectedId();
	char buf[256];
	snprintf(buf, sizeof(buf), "%s%s", m_prefix.c_str(),
			 selected_id >= 0 && selected_id < m_dummy->size() ?
				 (*m_dummy)[selected_id].text.c_str() :
				 "");

...
FileDialog::FileDialog(const IRect &rect, const char *title, Mode mode)
	...
	m_edit_box =
		make_shared<EditBox>(IRect(5, h - 49, w - 5, h - 26), 30, "", WindowStyle::gui_light);
...
void FileDialog::updateList() {
	...
	auto files = findFiles(m_dir_path, FindFileOpt::directory | FindFileOpt::regular_file |
										   FindFileOpt::relative | FindFileOpt::include_parent);

	std::sort(files.begin(), files.end());
	for(int n = 0; n < (int)files.size(); n++)
		m_list_box->addEntry(files[n].path.c_str(),
							 files[n].is_dir ? ColorId::yellow : ColorId::white);
}

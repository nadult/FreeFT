...
bool Window::isFocused() const { return m_is_focused && (!parent() || parent()->isFocused()); }

bool Window::isMouseOver() const { return isFocused() && m_is_mouse_over; }
...
void Window::process(const InputState &state) {
	...
	if(!is_handled) {
		if(m_dragging_mode && !is_handled) {
			...
			if(!is_handled)
				is_handled = onMouseDrag(state, m_drag_start, local_mouse_pos, m_dragging_mode - 1,
										 finished_dragging);
			if(!is_handled)
				is_handled =
					onMouseClick(state, local_mouse_pos, m_dragging_mode - 1, finished_dragging);
		}
		if(!is_handled) {
			if(m_has_inner_rect) {
	...
	if(escape && (!m_parent || m_is_popup) &&
	   !m_dragging_mode) // sending Event::escape only from main window
...
void Window::draw(Canvas2D &out) const {
	...
	if(m_has_inner_rect) {
		...
		if(isize.x > rsize.x) {
			...
			out.addFilledRect(IRect(spos * rsize.x, rsize.y - 5, (spos + epos) * rsize.x, rsize.y),
							  col2);
		}
		if(isize.y > rsize.y) {

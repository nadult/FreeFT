...
ListBox::ListBox(const IRect &rect, FColor color)
	: Window(rect, color), m_over_id(-1), m_dragging_id(-1),
...
int2 ListBox::visibleEntriesIds() const {
	...
	int end =
		min((offset.y + rect().height() + m_line_height) / m_line_height, (int)m_entries.size());
...
bool ListBox::onMouseDrag(const InputState &, int2 start, int2 end, int key, int is_final) {
	if(key == 0) {
		...
		if(is_final == 1 && m_over_id == m_dragging_id) {
			bool do_select = localRect().containsCell(end) &&
							 (!isPopup() || (m_over_id >= 0 && m_over_id < size()));
